...
Set up your JAGS model code for a state-space random walk with drift $x_t = x_{t-1} + u + w_t$ where $w_t \sim N(0, q)$. The observation model is $y_t = x_t + v_t$ where $v_t \sim N(0, r)$. This is very similar to the  [univariate state-space model](https://nwfsc-timeseries.github.io/atsa-labs/sec-jags-uss.html) in the lab book except that we are not fitting a AR(1) model rather a random walk with drift. Note that in the code, $u$ is `mu`, $\sqrt{q}$ is `sd.q` and $\sqrt{r}$ is `sd.r`.
...
8. Evaluate whether the model converged using R-hat values. See slides 40 & 41 in the [Bayesian lecture](https://nwfsc-timeseries.github.io/atsa/Lectures/Week%206/lec_11_bayes.html). For a JAGS model, just look at `print(mod_ss)`.
...
See examples in [section 12.2](https://nwfsc-timeseries.github.io/atsa-labs/sec-jags-lr-no-covariates.html) of the lab book.
