...
One nice thing about this approach is that it is robust to a fair bit of missing data. Here I delete a third of the data. I do this randomly throughout the dataset.  The data look pretty hopeless. No signal to be seen.
...
$$\begin{bmatrix}w_1 \\ w_2 \\ w_3\end{bmatrix}_t, \quad \begin{bmatrix}w_1 \\ w_2 \\ w_3\end{bmatrix}_t \sim MVN\left(0, \begin{bmatrix}1&0&0&0\\0&q_1&c_1&c_2 \\ 0&c_1&q_2&c_3 \\ 0&c_2&c_3&q_3\end{bmatrix}\right)$$
...
This example worked because I had a sensor that was quite a bit better than the others with a much smaller sd. I didn't know which one it was, but I did have at least one good sensor. If I up the sd on the first (good) sensor, then my signal estimate is not so good. The variance of the signal estimate is better than the average, but it is still bad. There is only so much that can be done when the sensor adds so much error.
...
Some more caveats are that I simulated data that was the same as the model that I fit, except the signal. However an AR-1 with $b$ and $q$ (sd) estimated is quite flexible and this will likely work for data that is roughly AR-1.  A common except is very smooth data that you get from sensor that record dense data (like every second).  That kind of sensor data may need to be subsample (every 10 or 20 or 30 data point) to get AR-1 like data.  
...
On the otherhand, I poised a difficult problem with two quite awful sensors. I sensor with a random walk error would be really alarming and hopefully you would not have that type of error.  But you might.  I can happen when local conditions are undergoing a random walk with slow reversion to the mean. Many natural systems look like that.  If you have that problem, subsampling that *random walk* sensor might be a good idea.
