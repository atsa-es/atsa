...
## Approach #1 (the rest of the class)

We estimate $m_t$.

If we had $m_t$ we can predict the mean $x_{t-1}$ but that error might also have some structure (think AR and MA processes) that we can use to help us predict future $x_t$ from $x_{t-1}$.  Our goal is to both model that error and estimate $m_t$.

## Approach #2 Box-Jenkins

We remove (detrend) $m_t$ from our data using differencing.  We model $\Delta_d x_t$ and then predict $\Delta_d x_{t+1}$. Once we have that we can predict $x_{t+1}$ by using $x_t$, $x_{t-1}$, etc with some algebra.

The error structure of $\Delta_d x_{t+1}$ is NOT the same as $e_t$. 
$$\Delta_d x_{t} = \phi_1\Delta_d x_{t-1}$ + \phi_2\Delta_d x_{t-2} + \dots + z_t$$

$z_t$ is the error of the differences.  And the $\phi$ are for the differences.  So it is more difficult to make a biological interpretation of the ARMA model.  Remember, the objective was to predict $x_{t+1}$.

## Examples of differencing, non-seasonal models
...
$$\Delta^2 x_t =  2\mu + w_t - 2w_{t-1} + w_{t-2}$$
So you expect that we should estimate a ARIMA(0,2,2) with drift.  MA(2) models require much data to estimate and this is often not the model that is selected by `auto.arima()`.
...
$$s_t = (\beta y^2-\beta y) f(month)$$
Then we need to take 3 seasonal differences to get rid of the seasonality.
