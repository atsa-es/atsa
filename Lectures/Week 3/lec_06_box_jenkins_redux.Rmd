...
We remove (detrend) $m_t$ from our data using differencing.  We model $\Delta^d x_t$ and then predict $\Delta^d x_{t+1}$. Once we have that we can predict $x_{t+1}$ by using $x_t$, $x_{t-1}$, etc with some algebra.
