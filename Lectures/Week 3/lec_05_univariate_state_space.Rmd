...
$$N_t = exp(u+e_t)N^b_{t-1}$$

$N_t$ is population size.

## Gompertz model

Talk the log, and we have

$$x_t = bx_{t-1}+u+e_t$$
$$e_t \sim N(0,q)$$
It is not required that $e_t$ is Gaussian but that is a common assumption. Dynamics of processes with non-Gaussian errors, esp long-tailed errors, is a common extension.  Autocorrelated errors could be implemented with MA process or covariates.
...
title("Weak density-dependence",cex.main=.75)
...
title("Strong density-dependence",cex.main=.75)
...
Often state-space models include inputs (explanatory variables).  and typically the $x$ or the $y$ is multivariate.
...
## univariate example: population count data
...
The observation model is $y_t=x+t+e_t$.
No “process” variability. All variability = “non-process or observation error”.
...
## Versus a state-space model

Autoregressive state-space models fit a RANDOM WALK through the data
variability = “observation error” + “process error”.
...
## Two types of variability. #1 Non-process
...
## The “non-process” variance is often unknowable in fisheries and ecological analyses
...
## Two types of variability. #2 Process

Process error is the difference between the expected $x_t$ given data up to time $t-1$ and the actual value at time $t$. “x” in the plot shows the expected $x_t$ given $x_{t-1}$.
...
```{r, echo=FALSE}
set.seed(123)
fit1 <- MARSS(log(wilddogs[,2]), model=list(R=matrix(0),tinitx=0), silent=TRUE)
fit2 <- MARSS(log(wilddogs[,2]), model=list(Q=matrix(0),tinitx=0), silent=TRUE)
...
## How can we separate process and observation variance? 
