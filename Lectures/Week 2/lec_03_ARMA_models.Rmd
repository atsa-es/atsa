...
$$
<br>
Thus, all AR(1) models where $|\phi| < 1$ are stationary  
...
## simulate AR(1)
AR1_pos <- arima.sim(n=50, model=AR_pos)
AR1_neg <- arima.sim(n=50, model=AR_neg)

## get y-limits for common plots
ylm <- c(min(AR1_pos,AR1_neg), max(AR1_pos,AR1_neg))

## set the margins & text size
par(mfrow=c(1,2), mai=c(0.8,0.8,0.2,0.2), oma=c(0,0,0,0))
## plot the ts
plot.ts(AR1_pos, ylim=ylm,
        ylab=expression(italic(x)[italic(t)]),
        main=expression(paste(phi[1]," = 0.7")))
...
## simulate AR(1)
AR1_bg <- arima.sim(n=50, model=AR_bg)
AR1_sm <- arima.sim(n=50, model=AR_sm)

## get y-limits for common plots
ylm <- c(min(AR1_bg,AR1_sm), max(AR1_bg,AR1_sm))

## set the margins & text size
par(mfrow=c(1,2), mai=c(0.8,0.8,0.2,0.2), oma=c(0,0,0,0))
## plot the ts
plot.ts(AR1_bg, ylim=ylm,
        ylab=expression(italic(x)[italic(t)]),
        main=expression(paste(phi[1]," = 0.9")))
plot.ts(AR1_sm, ylim=ylm,
        ylab=expression(italic(x)[italic(t)]),
        main=expression(paste(phi[1]," = 0.1")))
```
