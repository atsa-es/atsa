...
We can also show this by writing an MA(_q_) model with the backshift operator

$$
x_t = (1 + \theta_1 \mathbf{B} + \theta_2 \mathbf{B}^2 + \dots + \theta_q \mathbf{B}^q)w_t
$$
...
\begin{align}
  ...
  & \Downarrow \\
  w_t &= \frac{1}{1 - \theta\mathbf{B}} x_t \\[4pt]
  & \Downarrow \text{via Taylor Series} \\[4pt]
  w_t &= (1 + \theta\mathbf{B} + \theta^2 \mathbf{B}^2 + \dots + \theta^\infty\mathbf{B}^\infty) x_t \\
  w_t &= x_t + \theta x_{t-1} + \theta^2 x_{t-2} + \dots + \theta^\infty x_{t - \infty}
\end{align}
$$
...
<font size="10">NONSTATIONARY TIME SERIES MODELS</font>
