...
ARp <- list(c(0.7, 0.2, -0.1), c(-0.7, 0.2, 0.1))
...
for(p in 1:2) {
  ## assume SD=1, so not specified
  AR_mods[[p]] <- arima.sim(n=5000, list(ar=ARp[[p]]))
  plot.ts(AR_mods[[p]][1:50], las = 1,
          ylab = expression(italic(x[t])))
  acf(AR_mods[[p]], las = 1, main = "")
  mtext(side = 3, expr[[p]],
        line = 0.5, adj = 0.5)
  pacf(AR_mods[[p]], las = 1, main = "")
}
```
