...
$x_t = \phi_1 x_{t-1} + \phi_2 x_{t-2} \theta_1 w_{t-1} + \theta_2 w_{t-2} + w_t$

$\hat{x}_t = \phi_1 x_{t-1} + \phi_2 x_{t-2} \theta_1 w_{t-1} + \theta_2 w_{t-2}$
...
The **forecast** package provides a function to do standard residuals tests:
...
## Test for autocorrelation of time-series residuals

The standard test for autocorrelated time-series residuals is the Ljung-Box test.
...
$p>0.05$ would be interpreted as not enough statistical evidence to reject the null hypothesis. It doesn't mean they are independent but rather that you don't have support for dependent residuals.

## Summary

## Forecasting

The basic idea of forecasting with an ARIMA model to estimate the parameters and forecast foreward.

For example, let's say we want to forecast with ARIMA(2,1,0) model:
...
```{r}
fit <- forecast::Arima(anchovy, order=c(2,1,0))
coef(fit)
```
...
To get our forecast for 1988, we do this
