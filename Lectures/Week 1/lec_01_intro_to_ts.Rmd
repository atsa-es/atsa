...
plot.ts(AirPassengers, las = 1, ylab = "")
```
...
fltr <- c(1,1,1)/3
trend <- filter(AirPassengers, filter=fltr, method="convo", sides=2)
...
## weights for moving avg
fltr2 <- rep(1,9)/9
trend2 <- filter(AirPassengers, filter=fltr2, method="convo", sides=2)
lines(trend, col = "blue", lwd = 2)
...
## weights for moving avg
fltr3 <- rep(1,27)/27
trend3 <- filter(AirPassengers, filter=fltr3, method="convo", sides=2)
lines(trend, col = "blue", lwd = 2)
lines(trend2, col = "darkorange", lwd = 2)
...
## Classical decomposition | 2. Seasonal effect ($m_t$)

Once we have an estimate of of the trend $\hat{m}_t$, we can estimate $s_t$ simply by subtraction:
