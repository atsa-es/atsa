...
```{r plot_nile, fig.cap = "Flow of the River Nile"}
par(mai = c(0.9,0.9,0.1,0.1), omi = c(0,0,0,0))

plot.ts(Nile, las = 1, ylab = "")
```


## Example of linear filtering

```{r plot_nile_fltr1, fig.cap = "Flow of the River Nile"}
par(mai = c(0.9,0.9,0.1,0.1), omi = c(0,0,0,0))

plot.ts(Nile, las = 1, ylab = "")
## weights for moving avg
fltr <- c(1,1,1)/3
trend <- filter(Nile, filter=fltr, method="convo", sides=2)
lines(trend, col = "blue", lwd = 2)
text(x = 1960, y = max(trend, na.rm = TRUE),
...
```{r plot_nile_fltr2, fig.cap = "Flow of the River Nile"}
par(mai = c(0.9,0.9,0.1,0.1), omi = c(0,0,0,0))

plot.ts(Nile, las = 1, ylab = "")
## weights for moving avg
fltr2 <- rep(1,9)/9
trend2 <- filter(Nile, filter=fltr2, method="convo", sides=2)
lines(trend, col = "blue", lwd = 2)
lines(trend2, col = "darkorange", lwd = 2)
text(x = 1960, y = max(trend, na.rm = TRUE),
     labels = expression(paste(lambda, " = 1/3")),
     adj = c(0.5,0.5), col = "blue")
text(x = 1960, y = max(trend, na.rm = TRUE)*1.05,
     labels = expression(paste(lambda, " = 1/9")),
     adj = c(0.5,0.5), col = "darkorange")
```
