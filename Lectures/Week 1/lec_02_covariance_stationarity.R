...
mtext(side = 3, line = 0.4,
      at = (mean(xx) + par()$usr[1]) / 2, expression((italic(x[i]) - italic(m[x])) < 0))
mtext(side = 3, line = 0.4,
      at = (mean(xx) + par()$usr[2]) / 2, expression((italic(x[i]) - italic(m[x])) > 0))
mtext(side = 4, line = 0.5,
      at = (mean(yy) + par()$usr[3]) / 2, expression((italic(y[i]) - italic(m[y])) < 0),
      las = 1)
mtext(side = 4, line = 0.5,
      at = (mean(yy) + par()$usr[4]) / 2, expression((italic(y[i]) - italic(m[y])) > 0),
      las = 1)
...
nn <- 30
alpha <- 0.05
ts_SD <- qnorm(1-alpha/2, 0, 1) / sqrt(nn)

abline(h = -ts_SD, lty = "dashed", col = "blue")
text(x = 14, y = -0.55,
     expression(-frac(italic(z)[1 - frac(alpha, 2)], sqrt(italic(n)))),
     col = "blue")
abline(h = ts_SD, lty = "dashed", col = "blue")
text(x = 14, y = 0.55,
     expression(+frac(italic(z)[1 - frac(alpha, 2)], sqrt(italic(n)))),
