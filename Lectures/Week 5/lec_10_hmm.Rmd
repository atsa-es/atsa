...
`depmixS4` doesn't have a `predict()` or `forecast()` function, but
creating estimated data is pretty straightforward. We can get the means
out with
...
-   For example, if L_ochotensis time series was missing first half of
    values

-   The argument `ntimes` is a list that becomes particularly valuable
    here - representing the length of each time series
