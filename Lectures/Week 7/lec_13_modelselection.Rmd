---
title: "Model diagnostics and model selection"
author: ""
date: ""
output:
...
## Residual Diagnostics in State-Space Models

State-space models in `MARSS` assume:

- Observation equation: $y_t = Zx_t + a + v_t$
- State equation: $x_t = bx_{t-1} + u + w_t$

Where:
- $v_t \sim N(0, R)$: observation error
- $w_t \sim N(0, Q)$: process error
  
## What Types of Residuals Can We Get?

`MARSS()` provides:

- **Model residuals**: $$y_t - \hat{y}_t$$ 
  - How well the model predicts observations
  
- **State residuals**: $$x_t - E[x_t | y]$$ 
  - Deviations from estimated latent state 

## Residuals workflow

- For each of model and state residuals

- Plot residuals vs time

- Check ACF/PACF for autocorrelation

- QQ plot + histogram for normality

- Formal tests (optional): Shapiro-Wilk, Ljung-Box

## Residuals workflow

- `DHARMa` package has great built in diagnostics

- Not compatible with simulation steps in MARSS 

- But residual checks are straightforward and we can implement our own equivalents

## Example data: global ocean temperature anomalies
...
- simple univariate state space model
```{r echo = TRUE}
data("gtemp_ocean")
fit <- MARSS(gtemp_ocean)
```
...
resid <- MARSSresiduals(fit, type = c("tT", "tt1", "tt"), 
                        normalize = FALSE) 
```

Extract residuals of interest

- "tT" smoothed residuals conditioned on all data y

- "tt1" one-step ahead residuals, conditioned on data up to t-1 (innovation residuals)

- "tt" contemporaneous residuals conditioned on data up to time t

## Plotting model residuals

- Is there a trend? Are they stationary? 
```{r echo=TRUE, fig.height=3.5, fig.width=6}
resid_mod <- residuals(fit, type = "tT")
ggplot(resid_mod, aes(t, .resids, color = type)) + 
  geom_point() + xlab("Time") + ylab("Residuals") + 
  geom_hline(aes(yintercept = 0), col="black")
```

## ACFs and PCFs of residuals

- Spikes = autocorrelation

```{r echo=TRUE, fig.height=4, fig.width=6}
resid1 <- resid_mod[(resid_mod$type=="ytT"),]
acf(resid1$.resids, main = "ACF: Model Residuals")
```

## ACFs and PCFs of residuals

- PACF at lag 2: maybe we'd be better off with AR(2) model?

```{r echo=TRUE, fig.height=4, fig.width=6}
resid1 <- resid_mod[(resid_mod$type=="ytT"),]
pacf(resid1$.resids, main = "PACF: Model Residuals")
```
...
```{r echo=TRUE, fig.height=4, fig.width=6}
hist(resid1$.resid, 40,
     main = "Histogram of Model Residuals", xlab = "Residual")
```

## Extensions for multivariate models

- All of the previous diagnostics are appropriate for multivariate models

- Are residuals correlated across multiple time series?

- Why might this occur and how to fix it? 
...
## Bridging ML / Bayesian model selection

J.T. Thorson 2024: Measuring complexity for hierarchical models using effective degrees of freedom

- similar to `mgcv` in that we can approximate the effective degrees of freedom
- but applied to models with random effects / hierarchical structure
- has generally not been available in maximum likelihood
...
```{r echo=FALSE}
data("gtemp_ocean")
ar.global.1 = Arima(gtemp_ocean, order = c(1,1,1),
...
- All models will have forecast uncertainty increase with time
- Rate of increase depends on the model
```{r echo=TRUE}
plot(f1)
```
...
```{r echo=TRUE, results='markdown', fig.height=3.5}
