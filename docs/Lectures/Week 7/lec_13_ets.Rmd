...
Simple exponential smoothing uses this type of weighting that falls off exponentially and the objective is to estimate the best weighting ( $\alpha$ ):
...
------------- | ------------- | --------- |
ETS no trend | "ANN" | `ses()` |
ETS with trend  | "AAN" | `holt()` |
...
If we take the average of all one-step changes ($\Delta x_t = x_t - x_{t-1}$ we are using the average trend like the naive model with drift.  We put an equal weighting on all the $\Delta x_t$ in the data.

But we could use a weighting that falls off exponentially so that we more recent $\Delta x_t$ affect the forecast more than $\Delta x_t$ in the distant past.  That is what an ETS model with trend does.
...
The ETS model puts more weight on the recent $\Delta x_t$ (one-step trends in the data).

$$\hat{x}_{T+h} = l_T + h b_T$$

where $b_T$ is a weighted average with the more recent $\Delta x_t$ (trends) given more weight.
...
The estimated trend to use depends on the $\beta$ (weighting). In this case, it is a weighted average of past trends (`diff(dat)`) which make sense as the trend keeps changing.
...
legend("topright",c("diff(dat)","b(t)"),pch=c(2,NA),lty=c(0,1))
```
...
We will fit an an ETS model with trend to the training data and make a forecast for the years that we 'held out'. 
