...
where $y_t$ is the first difference of our anchovy data. $\mu = m(1 - \beta_1 - \beta_2)$.
...
* $d=0$: Arima(x, order=c(1,0,0), include.drift=TRUE, include.mean=FALSE)

$\mu$ is estimated and called `drift`.

$$x_t = \mu + \phi_1 x_{t-1} + w_t$$
* $d=0$: Arima(x, order=c(1,0,0), include.drift=TRUE, include.mean=TRUE)
...
* $d=1$: Arima(x, order=c(1,1,0), include.drift=TRUE)

$m$ is estimated and called `drift`. 

$$(y_t-m) = \phi_1 (y_{t-1}-m) + w_t$$
* $d=1$: Arima(x, order=c(1,1,0), include.drift=FALSE)

$$y_t = \phi_1 y_{t-1} + w_t$$

* $d=1$: Arima(x, order=c(0,1,0), include.drift=TRUE)
...
$$x_t = m + x_{t-1} + w_t$$
If $d\ge2$, then both `include.mean` and `include.drift` are ignored. $z_t = y_t-y_{t-1}$.

* $d=2$: Arima(x, order=c(1,2,0))
...
* $d=0$: arima(x, order=c(1,0,0), include.mean=TRUE)
...
* $d=1$: arima(x, order=c(1,1,0), include.mean=TRUE)

$$y_t = \phi_1 y_{t-1}+ w_t$$
* $d=1$: arima(x, order=c(0,1,0))
