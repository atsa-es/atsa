...
```{r install-tvarss, echo=TRUE, eval=FALSE}
...
ggplot(df, aes(x = iter, y = Parameter, group=chain, col=chain)) + 
  geom_line(alpha=0.7) + 
  theme_bw() +
  xlab("MCMC iteration")
```
...
data("harborSealWA")
```
```{r matplot-harborseal, fig.height=4}
...
```{r bayes-fit-dfa, echo=TRUE, cache=TRUE, fig.height=2, fig.align="center",eval=TRUE}
...
df <- data.frame(
  Time = sample(1:ncol(x), size = 100, replace = TRUE),
...
```{r fit-coho, echo=TRUE, cache=TRUE, results="hide", warning=FALSE, message=FALSE}
coho <- readRDS("coho_mean_length.rds")
# rename variables
...
ggplot(df, aes(nu)) +
  ...
  xlab("Student-t df") +
  ylab("Density") +
  geom_vline(aes(xintercept = median(df$nu))) +
  theme_bw()
```
...
* Gaussian Process models implement covariance functions, e.g. exponential, Gaussian, Matern
...
ggplot(gme, aes(Date, Close)) +
  geom_point() +
  geom_line() +
  theme_bw() +
  ylab("Closing GME price")
```
