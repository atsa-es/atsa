...
for(i in 1:length(lec_links3_html)){
  lec_links_html[i] <- paste(lec_links1_html[i], lec_links2_html[i], lec_links3_html[i], sep="<br>")
}
...
## topics formatted for html
vid_links_html <- sapply(lapply(vid_links, function(x){ tmp1<-"[Video]("; tmp2 <- ")"; tmp <- paste(tmp1,x,tmp2); tmp[x==""]=""; tmp=paste(tmp,collapse="<br>"); tmp}), c)
