...
vid_links_html <- sapply(lapply(vid_links, function(x){
  tmp1<-"[Panapto]("; tmp2 <- ")"; tmp <- paste(tmp1,x,tmp2); tmp[x==""]=""; tmp=paste(tmp,collapse="<br>"); tmp}), c)
...
colnames(syl_txt) <- c("Date", "Instructor", "Lecture topics",
                       "Slides", "Video of Lecture", "Slides with Audio", "Reading")
## generate table
kable(syl_txt, format = "html", caption = " ",
      align = "cclcccc", escape = FALSE) %>%
