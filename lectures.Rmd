...
lec_links <- lapply(week_lecs, function(x){if(is.list(x)){c(x$pdf,x$ppt,x$rmd)}else{""}})
## topics formatted for html
lec_links_html <- sapply(lapply(lec_links, function(x){ tmp <- paste(c("[PDF](","[PPT](","[Rmd]("),x,")<br>", collapse=""); tmp[x==""]=""}), c)
...
## topics formatted for html
vid_links_html <- sapply(lapply(lec_links, function(x){ tmp <- paste("[Video](",x,")<br>", collapse=""); tmp[x==""]=""}), c)
...
colnames(syl_txt) <- c("Date", "Instructor", "Lecture topics", "Presentation", "Video", "Reading")
## generate table
